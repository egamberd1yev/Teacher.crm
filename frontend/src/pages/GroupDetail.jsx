...
function AttendanceModal({ date, groupId, dark, onClose }) {
  ...
  return (
    <div
      onClick={onClose}
      style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"center", justifyContent:"center", padding:"16px", zIndex:100 }}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        style={{ background:bg, border:`1px solid ${border}`, borderRadius:"16px", padding:"24px", width:"100%", maxWidth:"360px", boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}
      >
        <p style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:"17px", fontWeight:700, color:text, margin:"0 0 4px" }}>
          Davomat
        </p>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"8px", maxHeight:"280px", overflowY:"auto" }}>
          ...
          {list.length === 0 && (
            <p style={{ fontSize:"13px", color:"#9CA3AF", textAlign:"center", padding:"16px 0" }}>O'quvchi yo'q</p>
          )}
        </div>

        <button
          onClick={onClose}
          style={{ marginTop:"16px", fontSize:"13px", color:sub, background:"none", border:"none", cursor:"pointer", fontFamily:"'Inter',sans-serif" }}
        >
          Yopish
        </button>
      </div>
    ...
    >
      ...
      >
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px", borderRadius:"10px", marginBottom:"20px", background: isPaid ? "#DCFCE7" : "#FEE2E2" }}>
          ...
          <span className={`s-pill ${isPaid ? "pill-green" : "pill-red"}`}>
            {isPaid ? "To'lagan" : "Qarzdor"}
          </span>
        </div>

        <button
          onClick={onToggle}
          style={{ width:"100%", padding:"11px", borderRadius:"10px", border:"none", cursor:"pointer", fontSize:"13.5px", fontWeight:600, color:"white", fontFamily:"'Inter',sans-serif", background: isPaid ? "#EF4444" : "#22C55E", transition:"opacity 0.15s" }}
        ...
        <button
          onClick={onClose}
          style={{ width:"100%", marginTop:"10px", padding:"8px", fontSize:"13px", color:sub, background:"none", border:"none", cursor:"pointer", fontFamily:"'Inter',sans-serif" }}
        >
          Yopish
        </button>
      </div>
...
export default function GroupDetail() {
  ...
  return (
    <Layout
      title={group.name}
      action={
        <Link to="/dashboard" style={{ fontSize:"13px", color:sub, textDecoration:"none" }}>
          ← Guruhlar
        </Link>
      }
    >
      <p style={{ fontSize:"13px", color:sub, marginTop:"-20px", marginBottom:"24px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"20px" }}>
        ...
        <div style={{ background:cardBg, border:`1px solid ${border}`, borderRadius:"12px", overflow:"hidden", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
          ...
          {showAdd && (
            <form
              ...
              style={{ padding:"14px 20px", borderBottom:`1px solid ${divider}`, background:formBg, display:"flex", flexDirection:"column", gap:"10px" }}
            >
          ...
          <div>
            ...
            {students.length === 0 && (
              <p style={{ padding:"24px 20px", textAlign:"center", fontSize:"13px", color:sub }}>
                Hali o'quvchi yo'q. + Qo'shish tugmasini bosing.
              </p>
            )}
          </div>
        </div>
        ...
        <div style={{ background:cardBg, border:`1px solid ${border}`, borderRadius:"12px", overflow:"hidden", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 20px", borderBottom:`1px solid ${divider}` }}>
            <button onClick={prevMonth} style={{ fontSize:"18px", color:sub, background:"none", border:"none", cursor:"pointer", lineHeight:1 }}>‹</button>
            <span style={{ fontSize:"13px", fontWeight:600, color:text }}>{MONTHS_UZ[viewMonth]} {viewYear}</span>
            <button onClick={nextMonth} style={{ fontSize:"18px", color:sub, background:"none", border:"none", cursor:"pointer", lineHeight:1 }}>›</button>
          </div>
      ...
      {payStudent && (
        <PaymentModal student={payStudent} month={mKey} dark={dark} onClose={() => setPayStudent(null)} onToggle={handleTogglePayment} />
      )}
    </Layout>
  );
}