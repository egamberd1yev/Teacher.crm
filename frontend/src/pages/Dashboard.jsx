...
export default function Dashboard() {
  ...
  const totalExpected  = Object.values(summaries).reduce((s, x) => s + Number(x?.expectedTotal  || 0), 0);
  const totalCollected = Object.values(summaries).reduce((s, x) => s + Number(x?.collectedTotal || 0), 0);
  const totalDebtors   = Object.values(summaries).reduce((s, x) => s + (x?.debtorsCount || 0), 0);

  const stats = [
    { label: "Kutilayotgan", value: `${totalExpected.toLocaleString()} so'm`,  sub: `${groups.length} ta guruh`, color: text },
    { label: "Yig'ilgan",    value: `${totalCollected.toLocaleString()} so'm`, sub: "bu oy",                    color: "#22C55E" },
    { label: "Qarzdorlar",   value: `${totalDebtors} ta`,                       sub: `${(totalExpected - totalCollected).toLocaleString()} so'm`, color: "#EF4444" },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "14px", marginBottom: "28px" }}>
        {stats.map((s) => (
          <div key={s.label} style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: "12px", padding: "18px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
            ...
            <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: "22px", fontWeight: 700, color: s.color, margin: "0 0 4px", letterSpacing: "-0.4px" }}>{s.value}</p>
            <p style={{ fontSize: "12px", color: subtext, margin: 0 }}>{s.sub}</p>
          </div>
        ))}
      </div>
